            ...
            if not logs.empty:
                ...
                def verify_log(row):
                    try:
                        ...
                        # DÜZELTME: Burada da FIXED_KEY kullanıyoruz
                        secret_salt = str(FIXED_KEY)
                        
                        raw = f"{ts}|{row['event_type']}|{row['username']}|{row['details']}|{secret_salt}"
                ...
                if 'checksum' in logs.columns:
                    ...
                    if "❌ CORRUPTED" in logs['Integrity Status'].values:
                        st.error("🚨 SECURITY ALERT: Some logs have been tampered with!")
                else:
                    st.warning("Old log format detected. Integrity check active for new logs.")
